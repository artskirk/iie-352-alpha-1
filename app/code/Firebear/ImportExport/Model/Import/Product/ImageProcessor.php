...
class ImageProcessor
{
    ...
    public function processMediaGalleryRows(
        ...
        if ($this->config['remove_images'] & !empty($rowData[Product::COL_STORE])
            ...
            foreach ($existingImages[$rowSku] as $keyImage => $image) {
                ...
                $stores = array_keys($existingAttributeImages[$rowData[Product::COL_SKU]][$keyImage]);
                unset($stores[array_search($rowData[Product::COL_STORE], $stores)]);
                if (!$stores) {
                    $removeImages[$keyImage] = $image;
                }
            }
            if ($removeImages) {
                $this->removeExistingImages($removeImages);
    ...
    public function getExistingAttributeImages($bunch): array
    {
        ...
        $query = $conn->select()->from(
            ['cpe' => $conn->getTableName('catalog_product_entity')],
            ['sku' => 'cpe.sku', 'store_code' => 'st.code', 'image' => 'cpev.value']
        )->join(
            ['cpev' => $conn->getTableName('catalog_product_entity_varchar')],
            'cpe.' . $this->getProductEntityLinkField() . '=' . 'cpev.' . $this->getProductEntityLinkField()
        )->join(['ea' => $conn->getTableName('eav_attribute')], 'cpev.attribute_id=ea.attribute_id')
            ->join(['st' => $conn->getTableName('store')], 'cpev.store_id=st.store_id')
            ->where('ea.attribute_code IN(' . $imageCodes . ')')
            ->where('cpe.sku IN(?)', implode(',', array_column($bunch, Product::COL_SKU)));
        $result = [];
        foreach ($conn->fetchAll($query) as $item) {
            $result[$item['sku']][$item['image']][$item['store_code']] = true;
        }
        return $result;
    }
}
